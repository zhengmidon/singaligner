...
from utils.data_utils import TIMITTextMelLoader, TIMITTextMelCollate
...
def prepare_test_dataloader(hparams):
    ...
    # testset = OperaTextMelLoader(hparams.validate_prefix, hparams, hparams.data_dir)
    # collate_fn = OperaTextMelCollate(hparams.data_dir)
    testset = TIMITTextMelLoader('TEST', hparams, hparams.data_dir)
...
def infer(model, dataloader, output_dir, hparams):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            text_padded, text_lengths, _, mel_lengths = x
            _, log_prob, _ = model(x)
    ...
    print(f"Average Duration Erros:{round(total_time / total_iters, 6)} s, Min Error {min_error} s, Max Error {max_error} s")
    out_f.write(f"\nAverage Duration Erros:{round(total_time / total_iters, 6)} s, Min Error {min_error} s, Max Error {max_error} s")
